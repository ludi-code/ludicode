/**
 * Methods relative to the level of a user
 */

$(document).ready(function () {

    function loadLevelList(userNames) {
        $.getJSON("v2/levelLists", function (data) {
    ...
    function loadUserNames() {
        var userNames = [];
        $.getJSON("v2/teachers", function (data) {
    ...
    function showLevels(data, userNames) {
        for (var i = 0; i < data.length; i++) {
            ...
            caption.append($('<h6>Auteur: <a href="profile.html?id=' + data[i].idAuthor + '&role=teacher" >'
